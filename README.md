...
- 补充 3x-ui 3.5.0 的 API Token 自动化方式，避免脚本 POST 登录接口遇到 CSRF `403`。
- 补充非交互安装后单独配置 HTTPS、写入 `webCertFile` / `webKeyFile` 的流程。
- 补充 3x-ui 3.5.0 的 `clients` / `client_inbounds` 表结构，确保 4 个入站共享一个 `admin` 客户端和同一个 `subId`。
- 补充 Xray Core `26.7.11` 降到 `26.6.27` 的实操注意，以及 `xray x25519` 新输出格式。
- 明确外部测试客户端下载失败时只能标记“待外部实测”，不能把端口监听当成节点已通。
